...
void ThneedModel::addNavInstructions(float *state, int state_size) {
  navInstructions = state;
}
...
void ThneedModel::execute() {
  if (!recorded) {
    thneed->record = true;
    if (use_extra) {
      float *inputs[7] = {recurrent, navInstructions, navFeatures, trafficConvention, desire, extra, input};
      thneed->copy_inputs(inputs);
    } else {
      float *inputs[6] = {recurrent, navInstructions, navFeatures, trafficConvention, desire, input};
      thneed->copy_inputs(inputs);
    }
    thneed->clexec();
