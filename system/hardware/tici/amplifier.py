...
class Amplifier:
  ...
  def _set_config_retry(self, config):
  ...
  def set_global_shutdown(self, amp_disabled):
    self._set_config_retry(AmpConfig("Global shutdown", 0b0 if amp_disabled else 0b1, 0x51, 7, 0b10000000))

  def initialize_configuration(self, model):
    self.set_global_shutdown(amp_disabled=True)

    for config in BASE_CONFIG:
      self._set_config_retry(config)
